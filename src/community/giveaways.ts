...
const gstart: Command = {
  ...
  async execute(interaction, ctx) {
    ...
    const msg = await interaction.channel.send({
      embeds: [embed(prize, winners, endAt, 0, interaction.user.tag)],
      components: [enterRow(id)],
    });
...
const gend: Command = {
  data: new SlashCommandBuilder()
    ...
    .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild)
    .addIntegerOption((o) =>
      o.setName('id').setDescription('Giveaway number').setRequired(true).setMinValue(1),
    ) as SlashCommandBuilder,
  async execute(interaction, ctx) {
    if (!interaction.inCachedGuild()) return;
...
const greroll: Command = {
  ...
  async execute(interaction, ctx) {
    ...
    if (channel && channel.type === ChannelType.GuildText) {
      await channel.send(
        `🎉 New draw for **${gw.prize}**: ${winners.map((w) => `<@${w}>`).join(', ')}!`,
      );
    }
...
const glist: Command = {
  ...
  async execute(interaction, ctx) {
    ...
    const lines = active.map(
      (g) => `#${g.id} — **${g.prize}** (ends <t:${Math.floor(g.endAt / 1000)}:R>)`,
    );
...
export async function handleGiveawayButton(
  ...
  if (gw.requiredRoleId && !interaction.member.roles.cache.has(gw.requiredRoleId)) {
    return void interaction.reply({
      content: `You need the <@&${gw.requiredRoleId}> role to enter.`,
      flags: MessageFlags.Ephemeral,
    });
  }
  ...
  await interaction.message
    .edit({
      embeds: [embed(gw.prize, gw.winners, gw.endAt, count, author?.tag ?? 'staff')],
      components: [enterRow(id)],
    })
    .catch(() => undefined);
}
...
export async function endGiveaway(ctx: AppContext, id: number): Promise<void> {
  ...
  if (gw.messageId) {
    ...
    await msg
      ?.edit({
        embeds: [
          embed(gw.prize, gw.winners, gw.endAt, entries.length, author?.tag ?? 'staff').setTitle(
            '🎉 Giveaway ended',
          ),
        ],
  ...
  if (winners.length) {
    ...
    await channel
      .send(`Nobody entered the giveaway for **${gw.prize}**. 😢`)
      .catch(() => undefined);
  }
