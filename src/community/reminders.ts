...
const remind: Command = {
  ...
  data: new SlashCommandBuilder()
    ...
    .addStringOption((o) =>
      o.setName('text').setDescription('What to remind you of').setRequired(true),
  ...
  async execute(interaction, ctx) {
    ...
    if (ms === null) {
      return void interaction.reply({
        content: 'Invalid time. E.g.: `10m`, `2h`, `1d`.',
    ...
    await interaction.reply({
      content: `Ok! I'll remind you in ${formatDuration(ms)}: "${text}"`,
      flags: MessageFlags.Ephemeral,
    });
  },
};
